...
SERVICE_FILE="/opt/advakod/unified_codex_system.service"
...
case "$1" in
    ...
    check-files)
        ...
        if [ -d "/opt/advakod/unified_codexes/codexes" ]; then
            file_count=$(ls -1 /opt/advakod/unified_codexes/codexes/*.pdf 2>/dev/null | wc -l)
            echo "📄 Скачано PDF файлов: $file_count"
            echo "📁 Размер директории: $(du -sh /opt/advakod/unified_codexes/codexes 2>/dev/null | cut -f1)"
            echo ""
            echo "📋 Список файлов:"
            ls -la /opt/advakod/unified_codexes/codexes/*.pdf 2>/dev/null || echo "Файлы не найдены"
        else
