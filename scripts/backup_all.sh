...
check_services() {
    ...
    # Проверка PostgreSQL (теперь на хосте)
    if pg_isready -h localhost -U advakod -d advakod_db >/dev/null 2>&1; then
...
backup_postgres() {
    ...
    if POSTGRES_HOST=localhost POSTGRES_PORT=5432 POSTGRES_DB=advakod_db POSTGRES_USER=advakod POSTGRES_PASSWORD="${POSTGRES_PASSWORD}" MAX_BACKUPS=30 bash "${SCRIPT_DIR}/backup_postgres.sh"; then
