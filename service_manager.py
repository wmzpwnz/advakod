...
class ServiceManager:
    ...
    async def _check_dependencies(self, service_name: str) -> bool:
        ...
        Для unified_rag зависимости не являются критическими - сервис может работать
        в режиме деградации без vector_store и embeddings
        """
        
        service_info = self._services[service_name]
        
        # Для unified_rag зависимости не критичны - можем инициализировать в режиме деградации
        if service_name == "unified_rag":
            missing_deps = []
            for dependency in service_info.dependencies:
                if dependency in self._services:
                    ...
                    # Зависимость не является управляемым сервисом
                    if not await self._check_external_dependency_via_manager(dependency):
                        missing_deps.append(dependency)
            
            if missing_deps:
                logger.warning("⚠️ Unified RAG будет инициализирован в режиме деградации (отсутствуют: %s)", 
                             ", ".join(missing_deps))
                # Разрешаем инициализацию, но сервис будет работать в режиме деградации
                return True
        
        # Для других сервисов зависимости критичны
        for dependency in service_info.dependencies:
            if dependency in self._services:
                dep_service = self._services[dependency]
                if dep_service.status not in [ServiceStatus.HEALTHY, ServiceStatus.DEGRADED]:
    ...
    async def _check_external_dependency(self, dependency: str) -> bool:
        ...
        if dependency == "vector_store":
            ...
            try:
                ...
                is_ready = vector_store_service.is_ready()
                if not is_ready:
                    logger.debug("Vector store не готов, но это не критично для базовой функциональности")
                return is_ready
            except Exception as e:
                logger.debug(f"Ошибка проверки vector_store: {e}")
                return False
            ...
            try:
                ...
                if hasattr(embeddings_service, '_model_loaded'):
                    is_loaded = embeddings_service._model_loaded
                elif hasattr(embeddings_service, 'is_model_loaded'):
                    is_loaded = embeddings_service.is_model_loaded()
                else:
                    # Пытаемся загрузить модель если не загружена
                    try:
                        if hasattr(embeddings_service, 'load_model'):
                ...
                if not is_loaded:
                    logger.debug("Embeddings модель не загружена, но это не критично для базовой функциональности")
                return is_loaded
            except Exception as e:
                logger.debug(f"Ошибка проверки embeddings: {e}")
                return False
