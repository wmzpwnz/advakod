...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Инициализируем unified LLM - принудительно загружаем модель
        try:
            # Принудительно сбрасываем флаг перед загрузкой
            unified_llm_service._model_loaded = False
            unified_llm_service.model = None
            await unified_llm_service.ensure_model_loaded_async()
            if unified_llm_service.is_model_loaded():
                logger.info("✅ Unified LLM service initialized - модель загружена")
            else:
                logger.warning("⚠️ Unified LLM service initialized - но модель не загружена")
        except Exception as e:
            logger.error(f"❌ Ошибка загрузки модели: {e}")
            logger.info("✅ Unified LLM service initialized (без модели)")
    ...
    try:
        ...
        # ВСЕГДА проверяем реальное состояние модели (объект должен существовать)
        model_actually_loaded = unified_llm_service.model is not None and unified_llm_service._model_loaded
        
        if not model_actually_loaded:
            logger.warning("⚠️ Model not loaded, attempting initialization...")
            # Принудительно сбрасываем флаг на случай, если он установлен неправильно
            unified_llm_service._model_loaded = False
            unified_llm_service.model = None
            await unified_llm_service.initialize()
            
            # Проверяем результат
            model_loaded_after = unified_llm_service.model is not None and unified_llm_service._model_loaded
            if model_loaded_after:
...
if settings.ENVIRONMENT == "production":
    # В продакшене используем конкретные домены + внутренние имена контейнеров
    ...
    # Добавляем внутренние имена контейнеров для Docker networking
    allowed_hosts.extend(["backend", "nginx", "advakod_backend", "advakod_nginx", "localhost", "127.0.0.1"])
