...
class ChatCache:
    ...
    @staticmethod
    async def cache_prompt(session_id: int, prompt: Any, ttl: int = 1800):
        """Кэширование подготовленного промпта для сессии"""
        key = cache_service.generate_key("prompt_cache", session_id)
        await cache_service.set(key, prompt, ttl)

    @staticmethod
    async def get_cached_prompt(session_id: int) -> Optional[Any]:
        """Получение кэшированного промпта для сессии"""
        key = cache_service.generate_key("prompt_cache", session_id)
        return await cache_service.get(key)
