...
# Контекст для проверки паролей с поддержкой pbkdf2 и bcrypt
...
class AuthService:
    ...
    def verify_password(self, plain_password: str, hashed_password: str) -> bool:
        """Проверка пароля с поддержкой pbkdf2 и bcrypt"""
        try:
            # Используем passlib для поддержки разных форматов (pbkdf2, bcrypt)
            return pwd_context.verify(plain_password, hashed_password)
        except Exception as e:
            # Fallback на bcrypt для обратной совместимости
            try:
    ...
    def get_current_user(self, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> User:
        ...
        try:
            ...
            if email is None:
                logger.warning(f"Token missing 'sub' field: {payload}")
            ...
            logger.warning(f"JWT decode error: {str(e)}, token length: {len(token) if token else 0}")
            raise credentials_exception
