...
# Внешняя зависимость llama_cpp (опционально для Backend)
try:
...
DEFAULT_SYSTEM_PROMPT = (
    "Ты опытный юрист-консультант по законодательству Российской Федерации. "
    "Отвечай профессионально, структурировано и строго на русском языке. "
    "Указывай применимые нормы права, статьи и актуальные даты, если они известны. "
    "Если данных недостаточно, честно скажи об этом и предложи уточнить детали."
)
...
def _fix_formatting(token: str) -> str:
    """Подчищает артефакты токенизации (2.СПб → 2. СПб и т.п.)."""
    if not token:
        return ""
    result = token
    for pattern, repl in _PUNCT_FIXES:
        result = re.sub(pattern, repl, result)
    # Исправляем случаи вроде "2.Санкт" → "2. Санкт"
...
def _is_preformatted_prompt(prompt: str) -> bool:
    """Проверяет, не прошёл ли вопрос предварительный препроцессинг."""
    if not prompt:
        return False
    lowered = prompt.lower()
    return "<|im_start|>" in prompt or ("вопрос:" in lowered and "ответ:" in lowered)
...
class UnifiedLLMService:
    ...
    def _load_model(self):
        ...
        with self._load_lock:
            ...
            try:
                ...
                # Оптимизированные параметры для ускорения генерации
                # n_batch - размер батча для обработки токенов (оптимально 256 для баланса скорости и памяти)
                n_batch = min(256, n_ctx)  # Уменьшен для стабильности
                self.model = Llama(
                    model_path=model_path,
                    n_ctx=n_ctx,
                    n_threads=n_threads,
                    n_gpu_layers=n_gpu_layers,
                    n_batch=n_batch,  # Оптимизация для batch processing
                    logits_all=False,
                    use_mmap=True,
                    use_mlock=False,  # Отключено для избежания проблем с памятью
                    verbose=False
                )
            ...
            def _blocking_call():
                try:
                    ...
                    try:
                        ...
                        optimized_top_k = getattr(settings, "VISTRAL_TOP_K", 40)
                        chat_res = self.model.create_chat_completion(
                            messages=[
                                {"role": "system", "content": "Юрист РФ. Отвечай чётко."},
    ...
    async def _stream_response_internal(self, request: LLMRequest) -> AsyncGenerator[str, None]:
        ...
        async with self._async_semaphore:
            ...
            temperature = request.temperature or getattr(settings, "VISTRAL_TEMPERATURE", 0.3)
            top_p = request.top_p or getattr(settings, "VISTRAL_TOP_P", 0.8)
            top_k = getattr(settings, "VISTRAL_TOP_K", 40)
            repeat_penalty = getattr(settings, "VISTRAL_REPEAT_PENALTY", 1.1)
            max_stream_tokens = min(allowed_max, getattr(settings, "VISTRAL_STREAM_MAX_TOKENS", 800))
            ...
            def worker():
                try:
                    logger.info(
                        "🚀 Streaming request %s (max_tokens=%s, temp=%s, top_p=%s)",
                        request.id,
                        max_stream_tokens,
                        temperature,
                        top_p,
                    ...
                    try:
                        ...
                        logger.warning("⚠️ chat-completion streaming failed: %s, fallback to text mode", chat_err)
                    ...
                    for chunk in stream_iter:
                        ...
                        formatted = _fix_formatting(delta)
                        if formatted:
            ...
            while True:
                ...
                if isinstance(token, str) and token.startswith("[ERROR]"):
                    raise RuntimeError(token[7:].strip() or "Streaming error")
                yield token

    def _prepare_prompt(self, question: str, context: Optional[str] = None) -> str:
        """Подготавливает промпт для юридических вопросов"""
        if _is_preformatted_prompt(question or ""):
            return question
        return self.create_legal_prompt(question, context)

    def create_legal_prompt(self, question: str, context: Optional[str] = None, chat_mode: Optional[str] = None) -> str:
        """Создание промпта для юридических вопросов (совместимо с instruct-моделями)."""
        system_prompt = self._build_system_prompt(None)  # System prompt без контекста
        question_text = (question or "").strip()
        if not question_text:
            question_text = "Опиши, какой юридический вопрос необходимо разобрать."
        ...
        # Если есть история диалога, вставляем её между system prompt и текущим вопросом
        if context and context.strip():
            # История уже в формате "Пользователь: ... Ассистент: ..."
            # Преобразуем в формат chatml
            ...
            for line in history_lines:
                ...
                if line.startswith('Пользователь:'):
                    user_msg = line.replace('Пользователь:', '').strip()
                    if user_msg:
                        parts.append("<|im_start|>user")
                        parts.append(user_msg)
                        parts.append("<|im_end|>")
                elif line.startswith('Ассистент:'):
                    assistant_msg = line.replace('Ассистент:', '').strip()
                    if assistant_msg:
    ...
    def _build_system_prompt(self, context: Optional[str]) -> str:
        """Строит system prompt. Контекст истории больше не добавляется сюда."""
        return DEFAULT_SYSTEM_PROMPT

    def _extract_user_message(self, raw_prompt: str) -> str:
        if not raw_prompt:
            return ""
        text = raw_prompt.strip()
        if "<|im_start|>user" in text:
            user_part = text.split("<|im_start|>user", 1)[1]
            user_part = user_part.split("<|im_end|>", 1)[0]
            return user_part.strip()
        if "Вопрос:" in text:
            segment = text.split("Вопрос:", 1)[1]
            if "Ответ:" in segment:
                segment = segment.split("Ответ:", 1)[0]
    ...
    def _build_chat_messages(self, request: LLMRequest) -> List[Dict[str, str]]:
        ...
        user_message = self._extract_user_message(request.raw_prompt)
        if not user_message:
            user_message = self._extract_user_message(request.prompt)
        if not user_message:
            user_message = "Разбери правовой вопрос и предложи решение."
        return [
